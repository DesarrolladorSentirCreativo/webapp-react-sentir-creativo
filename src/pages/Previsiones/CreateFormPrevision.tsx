...
const CreateFormPrevision: FC = () => {
  ...
  const formik = useFormik<ICreatePrevision>({
    ...
    onSubmit: async (values) => {
      setIsLoading(true)
      try {
        await previsionService.create(values, userId)
        getSuccess('La Previsión fue creada correctamente')
        navigate('/previsiones')
      } catch (e) {
        console.log(e)
        getError('La Previsión no pudo ser creada')
        setIsLoading(false)
      }
    }
  })
      ...
      <Card title="Formulario">
        <Box component="form" onSubmit={formik.handleSubmit}>
          <Typography variant="h5" sx={{ textAlign: 'center' }}>
            Formulario para Creación de Previsión
          </Typography>
...
export default CreateFormPrevision
