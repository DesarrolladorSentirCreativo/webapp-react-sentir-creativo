...
const UpdateFormCategoriaUserAdmin: FC = () => {
  const { id } = useParams()
  ...
  const loadCategoriaUserAdminData = async (
    categoriaUserAdmin: ICategoriaUserAdmin
  ): Promise<void> => {
    formik.setValues({
      id: categoriaUserAdmin.id,
      nombre: categoriaUserAdmin.nombre,
      descripcion: categoriaUserAdmin.descripcion
    })
  }
  ...
  const formik = useFormik<ICategoriaUserAdmin>({
    ...
    onSubmit: async (values) => {
      setIsLoading(true)
      try {
        await categoriaUserAdminService.update(values, userId)
        getSuccess('La categoría de usuario se actualizó correctamente')
        navigate('/categorias-usuarios')
      } catch (e) {
        console.log(e)
        getError('La categoría de usuario no se actualizó correctamente')
        setIsLoading(false)
      }
    }
  })
      ...
      <Card title="Formulario">
        <Box component="form" onSubmit={formik.handleSubmit}>
          <Typography variant="h5" sx={{ textAlign: 'center' }}>
            Formulario para Actualización de Categoría de Usuario
          </Typography>
...
export default UpdateFormCategoriaUserAdmin
